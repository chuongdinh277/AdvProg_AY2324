...
vector<string> filterWordsByLen(int wordLen, const vector<string>& vocabulary)
{
    ...
    for(string c : vocabulary)
        if(c.size()==wordLen) answer.push_back(c);
    return answer;
}
...
map<char, int> countOccurrences(const vector<string>& candidateWords)
{
    ...
    {
        string c =candidateWords[i];
        for(char ch : c) answer[ch]++;
    }
    return answer;
}
...
char findMostFrequentChar(const map<char, int>& occurrences, const set<char>& selectedChars)
{
    ...
    for(auto c : occurrences)
    {
        char ch = c.first;
        int m = c.second;
        if(selectedChars.find(ch) == selectedChars.end() && maxm<m)
        {
            answer=ch;
            maxm=m;
        }
...
char findBestChar(const vector<string>& candidateWords, const set<char>& selectedChars)
{
    ...
    //Write your code here
    map<char,int> occur;
    occur=countOccurrences(candidateWords);
...
bool wordConformToMask(const string& word, const string& mask, char ch)
{
    ...
    {
        if(word[i]!= '-' && word[i]!=ch && mask[i]!=ch) return false;
    }
    return answer=true;
}
...
vector<string> filterWordsByMask(const vector<string>& words, const string& mask, char ch)
{
    ...
    for(string c : words)
    {
        if(wordConformToMask(c,mask,ch)) answer.push_back(c);
    }
    return answer;
}
